...
function App() {
  ...
  const [mainImage, setMainImage] = useState(imagesList[0].imageUrl)
  ...
  const handleThumbnailClick = thumbnailUrl => {
    const correctImageObj = imagesList.find(item => item.imageUrl === mainImage)
    if (thumbnailUrl === correctImageObj.thumbnailUrl) {
      const nextImage =
        imagesList[Math.floor(Math.random() * imagesList.length)]
      setMainImage(nextImage.imageUrl)
  ...
  const onClickPlayAgain = () => {
    ...
    setTimer(60)
    setIsGameOver(false)
    setSelectedTab('FRUIT')
    setMainImage(imagesList[0].imageUrl)
  }
    ...
    <div className="app-container">
      ...
      {!isGameOver ? (
        <div className="game-container">
          <img src={mainImage} alt="match" className="main-image" />
          <ul className="tabs">
            {tabsList.map(tab => (
              <li key={tab.tabId}>
                <button
                  className={selectedTab === tab.tabId ? 'active-tab' : ''}
          ...
          <ul className="thumbnails-list">
            {filteredImages.map(each => (
              <li key={each.id}>
                <button onClick={() => handleThumbnailClick(each.thumbnailUrl)}>
                  <img
        ...
        <div className="score-card">
          ...
          <button className="reset-button" onClick={onClickPlayAgain}>
            <img
